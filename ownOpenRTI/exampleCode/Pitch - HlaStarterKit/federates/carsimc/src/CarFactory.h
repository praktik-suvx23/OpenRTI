 ...
 */
class CarFactory;
typedef std::shared_ptr<CarFactory> CarFactoryPtr;

class CarFactory {
public:
    explicit CarFactory(const std::string& carConfigDir);
     ...
     */
    void setCarConfigDir(const std::string& carConfigDir);

private:
    std::experimental::filesystem::path _carConfigDir;
};
