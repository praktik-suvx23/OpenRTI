...
int main() {
    try {
        ...
        // Define the RTI node host and port
        std::wstring host = L"localhost";  // replace with the actual host if needed
        std::wstring connectionString = host + L":14321";  // combine host and port into a single string

        // Connect to the RTI node (using a combined connection string)
        rtiAmbassador->connect(ambassador, rti1516e::HLA_EVOKED, connectionString);
