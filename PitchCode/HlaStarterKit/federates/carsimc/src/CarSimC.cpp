...
{
   std::shared_ptr<Simulator>  simulator(new Simulator(simConf));
   std::shared_ptr<HLAmodule>  hlaModule(new HLAmodule(simConf));
   std::shared_ptr<CarFactory> carFactory(new CarFactory(simConf.getCarsDir()));
...
{
   SimulatorConfig simConf("simulator.config");

   if (simConf.failedToLoad()) {
      wcout << L"Simulator configuration is bad" << endl;
      return 1;
   } else {
